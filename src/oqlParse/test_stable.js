...
class OQOTestRunnerStable {
    ...
    static runOQLToOQOFunc(test) {
        try {
            ...
            const result = OQOTestRunnerStable.queriesEqual(generatedOQO, test.query, test.ignore ?? []);
            if (!result.equal) {
    ...
    static runOQOToOQLFunc(test) {
        try {
            ...
            const result = OQOTestRunnerStable.queriesEqual(queryFromGeneratedOQL, test.query, test.ignore ?? []);
            if (!result.equal) {
    ...
    static async runNatLangFunc(test) {
        ...
        for (const prompt of test.natLang ?? []) {
            try {
                const oqo = await OQOTestRunnerStable.getNatLangQuery(prompt);
                const result = OQOTestRunnerStable.queriesEqual(oqo, test.query, test.ignore ?? []);
                if (!result.equal) {
                    result.expected = test.query;
                    result.actual = oqo;
                }
    ...
    async runTests(cases = ["oqlToQuery", "queryToOql", "natLang", "queryToSearch"]) {
        const testPromises = this.tests.map(async (test) => {
            ...
            if (cases.includes("oqlToQuery")) {
                const oqlToQueryResult = OQOTestRunnerStable.runOQLToOQOFunc(test);
                oqlToQueryResult.id = testId;
                this.onTestResultCb(oqlToQueryResult);
            }

            if (cases.includes("queryToOql")) {
                const queryToOqlResult = OQOTestRunnerStable.runOQOToOQLFunc(test);
                queryToOqlResult.id = testId;
                this.onTestResultCb(queryToOqlResult);
            }

            if (cases.includes("queryToSearch")) {
                const searchResult = await OQOTestRunnerStable.runSearchFunc(test);
            ...
            if ('natLang' in test && Array.isArray(test.natLang) && test.natLang.length > 0 && cases.includes("natLang")) {
                const natLangResult = await OQOTestRunnerStable.runNatLangFunc(test);
