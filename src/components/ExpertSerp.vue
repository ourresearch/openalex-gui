    ...
    <template v-if="mdAndUp">
      ...
      <v-row>
        <v-col cols="6">
          ...
          <!-- Filters: no filters, hidden (too few results), or normal -->
          ...
          <template v-else-if="showFilterBar">
...
const hasFiltersAvailable = computed(() => {
  const autoGeneratedKeys = new Set(['works_count', 'cited_by_count']);
  return facetConfigs(entityType.value).some(
    c => c.actions?.includes('filter') && !autoGeneratedKeys.has(c.key)
  );
});
// Hide the filter bar (including kebab) when too few results to be useful
const MIN_RESULTS_FOR_FILTERS = 100;
const showFilterBar = computed(() => {
  if (!hasFiltersAvailable.value) return false;
  if (route.query.filter || route.query['search.semantic']) return true;
  const count = resultsCount.value;
  if (count !== undefined && count < MIN_RESULTS_FOR_FILTERS) return false;
  return true;
});
