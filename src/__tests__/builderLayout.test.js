...
describe('layoutLines', () => {
  ...
  it('a leaf value-bag flows INLINE with its clause (one line, wraps in browser)', () => {
    // `title contains ( a or b or c )` — leaf group, no nesting → single line
    expect(lay([
      col('title'), op(' contains '), lp(), vb('a'), conn('or'), vb('b'), conn('or'), vb('c'), rp(),
    ])).toEqual(['0:title contains ( a or b or c )']);
  });

  it('the leaf bag is emitted as ONE grp item (its own wrap box)', () => {
    const lines = layoutLines([col('title'), op(' contains '), lp(), vb('a'), conn('or'), vb('b'), rp()]);
  ...
  it('block group (nested groups) explodes — one open paren per line', () => {
    ...
    expect(lay([
      kw('works'), kw(' where ', 'where'), col('title/abstract'), op(' contains '),
  ...
  it('deep 3-level nesting — every open paren alone on its line', () => {
    // `title contains ( ( (a or b) and c ) or d )`
    expect(lay([
      col('title'), op(' contains '),
